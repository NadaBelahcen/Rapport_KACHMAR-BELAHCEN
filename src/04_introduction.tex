...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.35\textwidth]{img/logo-afdtech.PNG}
    \caption{Logo d'AFD.Tech Part of Accenture}
    \label{fig:logo-afdtech.PNG}
\end{figure}
...
Le sujet de ce PFE s'intitule : \textit{« ADMP AI Studio : Modernisation et automatisation des pipelines ETL de migration de données via une architecture d'IA agentique »}.
...
\begin{itemize}
    \item Rester un différenciateur réel en matière de migration grâce à un moteur IA dédié (ADMP AI Engine).
    \item Optimiser et accélérer le temps nécessaire à la rédaction des spécifications de migration, et automatiser l'analyse des audits de données.
    \item Créer un véritable assistant intelligent pour les spécificateurs, réduisant la charge globale de travail.
\end{itemize}

Le périmètre de la mission couvre trois axes de contribution complémentaires, tous intégrés au sein de la plateforme ADMP AI Studio :

\begin{itemize}
    \item \textbf{L'Agent de Validation} : un module agentique implémenté via LangGraph, chargé de vérifier la cohérence et la complétude des livrables produits par les agents amont (Data Mapping, Table Mapping, Options Structurantes), selon une logique de décision GO/NO-GO avec boucle d'auto-correction pouvant aller jusqu'à trois itérations.
    \item \textbf{Les Load Reports et Dashboards} : des livrables de suivi générés automatiquement à l'issue de chaque cycle de validation, sous forme de rapports Excel détaillés et de tableaux de bord interactifs intégrés à l'interface de la plateforme.
    \item \textbf{Le package de génération Azure Data Factory (ADF)} : l'audit et la refonte du compilateur ADF existant au sein de Data Migration Studio, visant à rendre le mode Stored Procedure production-ready, avec correction de trois catégories de défauts (schéma \texttt{undefined}, authentification Managed Identity absente, Datasets superflus).
\end{itemize}

Ces contributions s'appuient sur l'expérience opérationnelle acquise lors de la participation active au projet de migration client CREPSA, qui a fourni le contexte métier concret permettant d'identifier les besoins réels auxquels l'ADMP AI Studio répond.
...
\begin{itemize}
    \item Comprendre et maîtriser la méthodologie ADMP et ses livrables (SDS, IDS, TDS, DS, RC, DP, TT, MS) à travers la formation Core ADMP Design School (CIDS).
    \item Contribuer opérationnellement au projet de migration CREPSA, notamment en matière d'analyse de données, de validation de tickets et de résolution d'anomalies.
    \item Concevoir et implémenter l'Agent de Validation avec sa boucle d'auto-correction dans le cadre de l'évolution IA de la plateforme.
    \item Produire les Load Reports et Dashboards permettant le pilotage de la qualité de migration.
    \item Auditer et refondre le package de génération Azure Data Factory pour le rendre déployable en environnement de production Azure.
    \item Tester et valider les solutions développées sur des cas réels ou simulés de migration.
    \item Documenter l'ensemble du travail réalisé dans le présent rapport de PFE.
\end{itemize}
